...
%   28/09/2011    SOH Madgwick    Initial release
    ...
    properties (Access = public)
        SamplePeriod = 1/256;
        Quaternion = [0.8 0.75 0 0]/norm([0.8 0.75 0 0]);     % output quaternion describing the Earth relative to the sensor
    ...
    methods (Access = public)
        ...
        function obj = UpdateIMUslt(obj, Gyroscope, Accelerometer)
            q = obj.Quaternion; % short name local variable for readability
            
            % set initial point for q
            ax=Accelerometer(1,3);
            ay=Accelerometer(1,2);
            az=Accelerometer(1,1);
           
            if az >= 0
                q=[sqrt((az+1)/2) -ay/sqrt(2*(az+1)) ax/sqrt(2*(az+1)) 0];
            else
                q=[-ay/sqrt(2*(1-az)) sqrt((1-az)/2) 0 ax/sqrt(2*(1-az))];
            end
        ...
        function obj = UpdateIMUslt2(obj, Gyroscope, Accelerometer)
            q = obj.Quaternion; % short name local variable for readability
            
            % set initial point for q
            q = [0.763523578361070  -0.0863212587840360  -0.639984676042049  0];
