...
if isstruct(gs) && ~isempty(fieldnames(gs))
    ...
    for k = 1:length(gs)
        ...
        % INCLUDED FOR TESTING ONLY, DON'T EXTRACT GSD        
        imu2 = imu.LowerBack.gyr(:,1);
...
%             [Turns]= turning_detection(imu_preprocessed, fc, fs, turnThres);
            [Turns]= turning_detection(imu2, fc, fs, turnThres);
            ...
            for i=1:length(Turns.Magnitude)
                ...
                TD(i+start_idx).Turn_AngularVelocity = imu2(Turns.TurnDur(i,1):Turns.TurnDur(i,2));
